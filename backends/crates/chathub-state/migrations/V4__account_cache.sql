...
DROP TABLE IF EXISTS hub_wecom_account_seqs;

-- 2) 重建 hub_wecom_accounts:旧 5 字段 → 新 8 字段契约,内部统一 employee_id
--    (业务后台 2026-05 重设的 listMine 响应形态:wecomAccountId/wecomName/wecomAccount/
--     wecomAlias/wecomAvatar/wecomStatus/gender/position)
DROP TABLE IF EXISTS hub_wecom_accounts;
CREATE TABLE hub_wecom_accounts (
...
CREATE INDEX idx_hub_wecom_accounts_employee ON hub_wecom_accounts(employee_id);
...
CREATE TABLE hub_wecom_account_watermark (
